...
function DraggableRow({ row, isGrouped }) {
    ...
    <TableRow ref={setNodeRef} style={style}>
      {row.getVisibleCells().map((cell, i) => {
        ...
        const commonCellProps = {
          ...
          ...(cell.getIsGrouped() && !cell.column.columnDef.meta && { style: { background: bgcolor } }),
          ...cell.column.columnDef.meta
        };

        if (cell.column.id === 'drag-handle' && !isGrouped) {
          return (
            <TableCell key={cell.id} {...commonCellProps}>
              <IconButton
                {...attributes}
                {...listeners}
                size="small"
...
export default function UmbrellaTable() {
  ...
  useEffect(() => {
    ...
    setColumnVisibility((prev) => ({ ...prev, 'drag-handle': !isGrouped }));
  }, [grouping]);
                  ...
                  {table.getRowModel().rows.length > 0 ? (
                    <SortableContext items={dataIds} strategy={verticalListSortingStrategy}>
                      {table.getRowModel().rows.map((row) => (
                        <Fragment key={row.id}>
                          <DraggableRow row={row} isGrouped={table.getState().grouping.length > 0} />
...
DraggableRow.propTypes = { row: PropTypes.object, isGrouped: PropTypes.bool };
